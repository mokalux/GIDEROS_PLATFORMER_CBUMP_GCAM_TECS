local atan2,sqrt,cos,sin,clamp,random = math.atan2,math.sqrt,math.cos,math.sin,math.clamp,math.random
local PI = math.pi
local INF = math.huge
...
local function outExponential(ratio)
	if ratio == 1 then return 1 end return 1-2^(-10 * ratio)
end
...
function GCam:init(content, ax, ay)
	...
	self.deadWidth  = 50
	self.deadHeight = 50
	self.deadHeightOffset = 0 -- XXX new 20220918
	self.deadRadius = 25
	-- Soft zone
	self.softWidth  = 150
	self.softHeight = 150
	...
	self.__debugRMesh:setIndexArray(
		1,3,4, 1,2,4, 1,3,7, 3,5,7, 2,4,8, 4,8,6, 5,6,8, 5,8,7, 9,10,11, 9,11,12, 13,14,15, 13,15,16, 17,18,19, 17,19,20
	)
	self.__debugRMesh:setColorArray(
		self.__debugSoftColor,self.__debugAlpha, self.__debugSoftColor,self.__debugAlpha, self.__debugSoftColor,self.__debugAlpha,
		self.__debugSoftColor,self.__debugAlpha,  self.__debugSoftColor,self.__debugAlpha, self.__debugSoftColor,self.__debugAlpha,
		self.__debugSoftColor,self.__debugAlpha, self.__debugSoftColor,self.__debugAlpha, self.__debugAnchorColor,self.__debugAlpha,
		self.__debugAnchorColor,self.__debugAlpha, self.__debugAnchorColor,self.__debugAlpha, self.__debugAnchorColor,self.__debugAlpha,
		self.__debugAnchorColor,self.__debugAlpha, self.__debugAnchorColor,self.__debugAlpha, self.__debugAnchorColor,self.__debugAlpha,
		self.__debugAnchorColor,self.__debugAlpha, self.__debugDotColor,self.__debugAlpha, self.__debugDotColor,self.__debugAlpha,
...
--			ax-dw,ay-dh, -- 3
--			ax+dw,ay-dh, -- 4
			ax-dw,ay-dh-self.deadHeightOffset, -- 3 XXX new 20220918
			ax+dw,ay-dh-self.deadHeightOffset, -- 4 XXX new 20220918
			ax-dw,ay+dh, -- 5
			ax+dw,ay+dh, -- 6
			
--			ax-sw,ay+sh, -- 7
--			ax+sw,ay+sh, -- 8
			ax-sw,ay+sh-self.deadHeightOffset, -- 7 XXX new 20220918
			ax+sw,ay+sh-self.deadHeightOffset, -- 8 XXX new 20220918
			
			ax-TS,-off, ax+TS,-off, -- vertical line
			ax+TS,h+off, ax-TS,h+off, -- vertical line
			
			-off,ay-TS, -off,ay+TS, -- horizontal line
			w+off,ay+TS, w+off,ay-TS -- horizontal line
		)
	...
	elseif self.shapeType == "circle" then
		...
		self.__debugCMesh:setVertexArray(
			0,0,0,0,0,0,0,0,ax-TS,-off, ax+TS,-off,ax+TS,h+off, ax-TS,h+off, -off,ay-TS, -off,ay+TS, w+off,ay+TS, w+off,ay-TS
		)
		self.__debugCMesh:setIndexArray(
			1,2,3, 1,3,4, 5,6,7, 5,7,8, 9,10,11, 9,11,12
		)
		self.__debugCMesh:setColorArray(
			self.__debugDotColor,self.__debugAlpha, self.__debugDotColor,self.__debugAlpha, self.__debugDotColor,self.__debugAlpha,
			self.__debugDotColor,self.__debugAlpha, self.__debugAnchorColor,self.__debugAlpha, self.__debugAnchorColor,self.__debugAlpha,
			self.__debugAnchorColor,self.__debugAlpha, self.__debugAnchorColor,self.__debugAlpha, self.__debugAnchorColor,self.__debugAlpha,
			self.__debugAnchorColor,self.__debugAlpha, self.__debugAnchorColor,self.__debugAlpha, self.__debugAnchorColor,self.__debugAlpha
		)
...
function GCam:debugUpdate(dotOnly, gx,gy) -- a green little pixel :-)
...
--	elseif y < self.y - dh then  -- out of dead zone on top side
	elseif y < self.y - dh - self.deadHeightOffset then  -- out of dead zone on top side XXX new 20220918
		local t = smoothOver(dt, self.smoothY, 0.99)
--		local newY = lerp(self.y, y + dh, t)
		local newY = lerp(self.y, y + dh + self.deadHeightOffset, t)
		dstY = clamp(newY, y - sh,y + sh)
	end
	-- clamp to soft zone
...
function GCam:setFollow(obj)
	if (obj ~= nil) then 
--		assert(obj.getPosition and obj:getClass() == 'Sprite', "Invalid follow object!") -- new for tiny 20220927 XXX
...
function GCam:setDeadSize(w,h,o)
	self.deadWidth = w
	self.deadHeight = h or w
	self.deadHeightOffset = o or 0
	self:debugUpdate()
end
...
function GCam:setDeadHeightOffset(o) -- XXX new 20220918
	self.deadHeightOffset = o
	self:debugUpdate()
end
...
function GCam:getAnchorX()
	return self.ax
end
function GCam:getAnchorY()
	return self.ay
end
